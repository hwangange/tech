<?php
	...
	class View {
		...
		public function find_Form($database, $uniqueIDmessage){
	...
	$view = new View();
	$data = array();
	if(isset($_REQUEST['database'], $_REQUEST['uniqueIDmessage'])) {
		$database = $_REQUEST['database'];
		$uniqueIDmessage = $_REQUEST['uniqueIDmessage'];

		if(!empty($database)&&!empty($uniqueIDmessage)) {
		
			$view -> find_Form($database, $uniqueIDmessage);
		}else {
